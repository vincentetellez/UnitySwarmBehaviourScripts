...
public class Swarm : AIBehaviour
{
	...
	void CalcSeparation() {
		...
		// Calculates the separation between all swarm neighbors so that a proper distance can be maintained. This method accounts for separation only.
		...
		for ( int i = 0; i < swarmList[swarmNumber].Count; i++ ) {
			...
			if ( neighbor != this.gameObject ) {
                ...
                //hiddenWeight = -preferredSeparation / Mathf.Max( dist.sqrMagnitude, divZeroAccuracy );
                //separation += hiddenWeight * dist.normalized;
                if ( dist.sqrMagnitude < preferredSeparation ) {
                    separation += -dist;
                }
			}
		}
    ...
    void CalcLocalSeparation() {
		...
		// Calculates the separation between closest swarm neighbors so that a proper distance can be maintained. This method accounts for separation only.
